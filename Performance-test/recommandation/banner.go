...
func SyntheticRecommendRequests() chan interface{}{
	...
	f, err := os.Open("/home/ec2-user/qa-test/pressure/mx-beta/pressure/data/banner.log")
	if err != nil {
		fmt.Print(err)
	}
	...
	for ; len(c)<num-1; {
		...
		for _, info := range lineArray {
			...
			if "localFileInfo"==everyInfo[0]{
				for i,info2:=range everyInfo{

					if strings.Contains(info2,"fileName"){
						infoMap["fileName"]=everyInfo[i+1]
					}
				}
				continue
...
key := strings.Replace(everyInfo[0], " ", "", -1)
		...
		_, ok = infoMap["tabId"]
		if ok {
			request.TabId = "5bf68b89b8dc9fce0bfb567750f69795"
		}else{
			request.TabId = ""
		}
...
func ASExecutor(request interface{}, transport thrift.TTransport) (interface{}, error) {
	...
	return client.FetchBanner(nil, request.(*recommend.Request))
